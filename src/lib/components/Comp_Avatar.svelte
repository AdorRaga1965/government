<script lang="ts">
  ...
  // Reactive declaration to re-generate avatar whenever props change
  ...
  /**
   * Generates an SVG string for a DiceBear avatar based on user properties.
   ...
   * @returns The SVG string for the generated avatar.
   */
  function generateAvatarSvg(user: { fullname: string; age: number; gender: string }): string {
    ...
    if (user.age < 18) {
      avatarOptions.scale = 80; // Make the avatar slightly smaller to imply youth
    ...
    // Generate the avatar SVG string
